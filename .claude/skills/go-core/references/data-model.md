...
All models are in `pkg/models/`. The project uses PostgreSQL with pgx/SQLC. Models in `pkg/models/` define the schema structure. SQLC-generated types in `internal/sqlcgen/` are used for queries. All models except SystemSetting and SchemaMigration use UUID primary keys (`gen_random_uuid()`). Sensitive fields use `json:"-"` to prevent API exposure.
