...
func main() {
	...
	// Connect to database (pgx pool)
	dbPort, _ := strconv.Atoi(os.Getenv("DB_PORT"))
	if dbPort == 0 {
		dbPort = 5432
	}
	pgxPool, err := database.ConnectPgx(
		os.Getenv("DB_HOST"),
		dbPort,
		os.Getenv("DB_USER"),
	...
	repo := admin.NewAccountRepository(pgxPool)
	...
	if existing != nil {
		...
		if err := repo.UpdatePassword(existing.ID.String(), string(hashedPassword)); err != nil {
			log.Fatalf("Failed to update admin account password: %v", err)
		}
		if adminEmail != "" {
			if err := repo.UpdateEmail(existing.ID.String(), adminEmail); err != nil {
				log.Fatalf("Failed to update admin account email: %v", err)
			}
		}
		fmt.Println()
		fmt.Println("===========================================")
