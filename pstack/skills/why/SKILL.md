---
name: why
description: "Use for 'why does X work this way', 'why we picked Y', design rationale, regressions, postmortems, or data-backed thresholds. Queries git and gh (PRs and GitHub Issues), plus incident-postmortem when the code looks defensive, then returns a cited read on decisions and tradeoffs. Use how for runtime behavior."
...
Historical context in this repo lives in git, GitHub PRs, GitHub Issues, in-repo docs, and comments. Always search those. Only spawn extra investigators when an MCP for chat, tickets, observability, or analytics is actually enabled in this session. Null results from searched sources are first-class evidence. Report them alongside positive findings.
...
**Default to the sources this repo can query.** Always run source control (git + `gh` for PRs and GitHub Issues). Add the incident-postmortem playbook when the target looks defensive. Only spawn extra investigators when an MCP for that category is actually enabled in this session. Do not look for Linear, Notion, Slack, Datadog, Sentry, or Databricks playbooks. Those files are not in this snapshot.

### Discovery

Before spawning investigators, list the available MCPs from the Cursor environment. Use the available-tools map when present.

Source control is always available through git and `gh`. For any extra MCP, classify it from the server name and tool names. Record skipped categories as gaps.

Launch matching investigators in a single message so they run concurrently. One investigator per category.

Subagent config (each):
- `subagent_type`: `generalPurpose`
- `model`: the `why investigators` slug from `.cursor/rules/pstack-models.mdc` (then `~/.cursor/rules/pstack-models.mdc` if the project file is missing)
- `readonly`: `false` (agent mode). **Do not use readonly/Ask mode.** It strips MCP access. Investigators still shouldn't write anything. That's a posture, not a sandbox.

Each investigator gets:
1. The base prompt from `references/investigator-prompt.md`
2. The category playbook `references/sources/code-archaeology.md` for git/`gh`, or an ad-hoc adaptation if an extra MCP is actually present
...
1. **Source control investigator**. Git history, `gh` for PRs and GitHub Issues, code comments, tests, in-repo docs. Always spawn. Best at surfacing implementation-time rationale captured during review.

Skip other categories unless a matching MCP is enabled. Record the skip as a gap, not a choice.

### When to skip an investigator

Only skip with an **explicit, written justification** that goes in the final "Sources Consulted" section. Two valid reasons:

- **No MCP is available for that category** in this environment. Flag this as a gap, not a choice.
- **The source is provably irrelevant**, not just "probably irrelevant." A high bar.

If your scope assessment suggests a single-commit trivial target where the PR description already contains the complete answer, you may answer inline **only after** confirming git/`gh` would be redundant. Say so explicitly. This should be rare.
...
- `subagent_type`: `generalPurpose`
- `model`: the `why synthesizer` slug from `.cursor/rules/pstack-models.mdc` (then `~/.cursor/rules/pstack-models.mdc` if the project file is missing)
...
Example:
- Source control (git/gh): `git log --follow internal/user/service.go`, PRs #49, #50. Found PR #50 bound admin keys to `app_id`.
- GitHub Issues: searched linked issues from those PRs. No separate ticket discussing the last-superadmin race.
- Extra MCPs: skipped. None for chat, observability, or analytics in this environment.
...
- **Skipping investigators by anticipation**. Deciding up front that git/`gh` is enough because the PR body looks complete, without searching linked issues and in-repo docs. Always search source control. A null result is a data point; a skipped search is a blind spot.
...
- `references/sources/*.md`. `code-archaeology.md` for git/`gh`, plus cross-cutting `incident-postmortem.md`. Do not look for deleted Linear/Notion/Slack/Datadog/Sentry/Databricks playbooks.
