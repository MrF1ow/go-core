...
4. Pick the worker model from `swarm workers` in `.cursor/rules/pstack-models.mdc` (then `~/.cursor/rules/pstack-models.mdc` if the project file is missing). For a model race, name each arm's model up front. Do not use inline fallback slugs.
