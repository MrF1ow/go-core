...
**Defaults for every `Task` call.** `run_in_background: true`, agent mode (readonly strips MCP), file pointers not inlined context, explicit model per role from `.cursor/rules/pstack-models.mdc` (copied to `~/.cursor/rules/pstack-models.mdc` on Cloud Agent start). Read that file. Do not use this skill's old inline slugs (fable, opus 5, `-fast`, `-max`, `-xhigh`). A role line of `inherit-parent` or `auto` runs that role on the parent chat model (omit Task `model`).
