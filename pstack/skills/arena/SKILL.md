...
3. Pick the runners. Use `arena runners` from `.cursor/rules/pstack-models.mdc` (then `~/.cursor/rules/pstack-models.mdc` if the project file is missing). Spawn more when the arena covers multiple design directions. Same model N times when the work is generation-bound rather than judgment-sensitive. Do not use inline fallback slugs.
