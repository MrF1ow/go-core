...
func TestMain(m *testing.M) {
	...
	Init("test-jwt-secret-that-is-at-least-32-bytes-long!", 15*time.Minute, 720*time.Hour)
...
func TestParseTokenExpired(t *testing.T) {
	appID := "00000000-0000-0000-0000-000000000001"
	userID := "test-user-id"
	// Generate a token with a very short TTL so it expires immediately
	token, err := GenerateAccessToken(appID, userID, "", nil, 1*time.Millisecond)
	if err != nil {
		t.Fatalf("Failed to generate token: %v", err)
	}
