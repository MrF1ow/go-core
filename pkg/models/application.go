...
type Application struct {
	...
	// Email link paths — per-app path suffixes appended to FrontendURL when building
	// action links sent in transactional emails. Falls back to hardcoded defaults when empty.
	// Examples: "/auth/reset-password", "/account/verify", "/login/magic"
	ResetPasswordPath string `json:"reset_password_path"` // Default: /reset-password
	MagicLinkPath     string `json:"magic_link_path"`     // Default: /magic-link
	...
	RefreshTokenTTLHours  int `json:"refresh_token_ttl_hours"`  // Refresh token lifetime in hours (0 = use REFRESH_TOKEN_EXPIRATION_HOURS)

	CreatedAt            time.Time             `json:"created_at"`
	UpdatedAt            time.Time             `json:"updated_at"`
	OAuthProviderConfigs []OAuthProviderConfig `json:"oauth_provider_configs"`
	EmailServerConfig    *EmailServerConfig    `json:"email_server_config,omitempty"`
