...
- Tests in `internal/middleware`: env principal can `tenants:write`; viewer principal cannot; missing context key denies; type-assert failure denies.
...
Runtime: httptest of middleware only, not full `/admin/tenants`. Prove env principal and viewer principal through `RequireOperatorPermission`.
