...
- Add `web/templates/partials/forbidden_fragment.tmpl`. Any other HTMX target uses it. Outer id is the sanitized target. Crafted `HX-Target` that fails `[A-Za-z][A-Za-z0-9_-]{0,127}` is treated as a page deny.
...
- HTMX deny with target `#tenant-table` is the fragment whose outer id is `tenant-table`.
