...
import (
	...
	healthpkg "github.com/JedidiahDigital/go-core/internal/health"
	logService "github.com/JedidiahDigital/go-core/internal/log"
	oidcpkg "github.com/JedidiahDigital/go-core/internal/oidc"
	...
	passkeypkg "github.com/JedidiahDigital/go-core/internal/webauthn"
...
func (h *GUIHandler) sessionMaxAgeSeconds() int {
	return int(h.AdminSessionTTL.Seconds())
}
...
func (h *GUIHandler) SessionList(c *gin.Context) {
	...
	activeThresholdMinutes := int(h.AccessTokenTTL.Minutes())
...
func (h *GUIHandler) UserImport(c *gin.Context) {
	...
	if parseErr == nil && result.Imported > 0 {
		...
		if len(emails) > 0 {
			if createdUsers, err := h.Repo.GetUsersByEmailsAndApp(emails, appUUID); err == nil {
