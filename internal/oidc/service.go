...
import (
	...
	userpkg "github.com/JedidiahDigital/go-core/internal/user"
...
func NewService(repo *Repository, roleLookup RoleLookupFunc, publicURL string, authCodeTTL, idTokenTTL, accessTokenTTL time.Duration) *Service {
	return &Service{
		repo:           repo,
...
func (s *Service) MintTokensForUser(app *models.Application, client *models.OIDCClient, user *models.User, scopes []string, nonce string) (accessToken, refreshToken, idToken string, expiresIn int, err error) {
	...
	idTokenDur := time.Duration(app.OIDCIDTokenTTL) * time.Second
	if idTokenDur <= 0 {
		idTokenDur = s.IDTokenTTL
	}
	if idTokenDur <= 0 {
		idTokenDur = 3600 * time.Second
	}
	...
	if accessTTLSec <= 0 {
		accessTTLSec = int(s.AccessTokenTTL.Seconds())
	}
	if accessTTLSec <= 0 {
		accessTTLSec = 900
	}
...
func (s *Service) ClientCredentialsGrant(app *models.Application, clientID, clientSecret, scopeStr string) (accessToken string, expiresIn int, err error) {
	...
	if ccAccessTTLSec <= 0 {
		ccAccessTTLSec = int(s.AccessTokenTTL.Seconds())
	}
	if ccAccessTTLSec <= 0 {
		ccAccessTTLSec = 900
	}
