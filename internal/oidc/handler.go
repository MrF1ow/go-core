...
import (
	...
	"github.com/JedidiahDigital/go-core/internal/log"
...
func NewHandler(svc *Service, repo *Repository, publicURL string, accessTokenTTL time.Duration, jwtSecret string) *Handler {
	return &Handler{
		Service:        svc,
		Repo:           repo,
		PublicURL:      publicURL,
...
func (h *Handler) issueCodeAndRedirectForUser(c *gin.Context, app *models.Application, client *models.OIDCClient, userID string, req *dto.OIDCAuthorizeRequest) {
	...
	cookieDomain := ""
	if h.PublicURL != "" {
		if parsed, err := url.Parse(h.PublicURL); err == nil {
			cookieDomain = parsed.Hostname()
		}
	}
	if sessionToken != "" {
		c.SetCookie(cookieName, sessionToken, int(browserSessionTTL/time.Second), "/", cookieDomain, strings.HasPrefix(h.PublicURL, "https://"), true)
	}
