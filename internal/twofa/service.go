...
func NewService(userRepo *user.Repository, emailService *emailpkg.Service) *Service {
	return &Service{
		UserRepo:     userRepo,
...
func (s *Service) checkTwoFAEnabled(appID uuid.UUID) *errors.AppError {
	if appPtr, lookupErr := s.AppLookup(appID.String()); lookupErr == nil && appPtr != nil && !appPtr.TwoFAEnabled {
