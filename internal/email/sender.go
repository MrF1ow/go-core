...
// SMTPEmailSender wraps the existing Sender with a fixed SMTP config,
// implementing the core.EmailSender interface.
type SMTPEmailSender struct {
	sender *Sender
	config SMTPConfig
}

// NewSMTPEmailSender creates an EmailSender that uses the provided SMTP config.
...
// Send implements core.EmailSender.
func (s *SMTPEmailSender) Send(_ context.Context, to, subject, htmlBody, textBody string) error {
	return s.sender.Send(s.config, to, subject, htmlBody, textBody)
}
