...
type Service struct {
	...
	// ExternalSender, when set, overrides the internal SMTP sender for all
	// email delivery. The service still handles template resolution and
	// rendering; only the transport changes. This allows consuming apps to
	// plug in their own email provider (SES, SendGrid, etc.).
	ExternalSender core.EmailSender
}
...
// Resolver returns the VariableResolver so callers (e.g. main.go) can wire
// lookup callbacks after construction.
...
func (s *Service) SendEmailWithContext(appID uuid.UUID, emailTypeCode string, toEmail string, userID *uuid.UUID, vars map[string]string) error {
	...
	// 4. Send email
	return s.send(smtpConfig, toEmail, subject, htmlBody, textBody)
}
...
func (s *Service) SendVerificationEmail(appID uuid.UUID, toEmail, token string, userID *uuid.UUID) error {
	...
	if s.resolver.LookupApp != nil {
		if lookedUp, err := s.resolver.LookupApp(appID.String()); err == nil && lookedUp != nil {
			app = *lookedUp
		}
	}
...
// send routes email delivery through the ExternalSender if configured,
// otherwise falls back to the internal SMTP sender.
func (s *Service) send(smtpConfig SMTPConfig, to, subject, htmlBody, textBody string) error {
	if s.ExternalSender != nil {
		return s.ExternalSender.Send(context.Background(), to, subject, htmlBody, textBody)
	}
	return s.sender.Send(smtpConfig, to, subject, htmlBody, textBody)
}

// sendTest routes test email delivery through the ExternalSender if configured,
// otherwise uses the internal sender's SendTest (which always returns errors).
func (s *Service) sendTest(smtpConfig SMTPConfig, to, subject, htmlBody, textBody string) error {
	if s.ExternalSender != nil {
		return s.ExternalSender.Send(context.Background(), to, subject, htmlBody, textBody)
	}
	return s.sender.SendTest(smtpConfig, to, subject, htmlBody, textBody)
}
...
<p>If you received this email, your SMTP configuration is working correctly.</p>
	...
	return s.sendTest(smtpConfig, toEmail, subject, htmlBody, textBody)
}
