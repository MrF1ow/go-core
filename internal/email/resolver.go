...
type VariableResolver struct {
	LookupEmailType func(code string) (*models.EmailType, error)
	LookupUser      func(userID string) (*models.User, error)
	LookupApp       func(appID string) (*models.Application, error)
}

// NewVariableResolver creates a new resolver with callback-based lookups.
...
func (r *VariableResolver) applyStaticDefaults(vars map[string]string, emailTypeCode string) {
	if r.LookupEmailType == nil {
		return
	}

	emailType, err := r.LookupEmailType(emailTypeCode)
...
func (r *VariableResolver) applyUserVars(vars map[string]string, userID uuid.UUID) {
	...
	user, err := r.LookupUser(userID.String())
	if err != nil || user == nil {
...
func (r *VariableResolver) resolveAppName(appID uuid.UUID) string {
	if r.LookupApp != nil {
		if app, err := r.LookupApp(appID.String()); err == nil && app != nil && app.Name != "" {
			return app.Name
		}
	}
...
func (r *VariableResolver) resolveAppFrontendURL(appID uuid.UUID) string {
	if r.LookupApp != nil {
		if app, err := r.LookupApp(appID.String()); err == nil && app != nil {
