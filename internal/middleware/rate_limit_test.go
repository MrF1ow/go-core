...
func clearRateLimitState(keyPrefixes ...string) {
	...
	// 2. Clear cache rate-limit keys (if cache store is available).
	if cacheStore == nil {
		return
	}
	ctx := context.Background()
	if err := cacheStore.Ping(ctx); err != nil {
		return // Cache not reachable — nothing to clean.
	}
	for _, prefix := range keyPrefixes {
		...
		var cursor uint64
		for {
			keys, next, err := cacheStore.Scan(ctx, cursor, pattern, 100)
			if err != nil {
				break
			}
			if len(keys) > 0 {
