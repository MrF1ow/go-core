...
// Set via SetCacheStore during application startup; when nil, the middleware
// falls back to the in-memory store for all requests.
var cacheStore core.CacheStore

// SetCacheStore sets the CacheStore used by all rate-limit middleware.
// Call this during application bootstrap (e.g. in main.go) before
// registering any routes that use rate limiting.
func SetCacheStore(store core.CacheStore) {
	cacheStore = store
}
...
func RateLimitMiddleware(cfg RateLimitConfig) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// ---------------------------------------------------------------
		// Try CacheStore first (Redis or other backend)
		// ---------------------------------------------------------------
		if cacheStore != nil {
			if cacheOK := tryCache(c, cfg, attemptsKey, lockoutKey); cacheOK {
				return // CacheStore handled it (either allowed or rate-limited).
			}
		}
...
// tryCache performs the full rate-limit check via the CacheStore. Returns true
// if it was able to complete (whether allowed or rate-limited). Returns false
// if a cache error occurred and the caller should fall back to in-memory.
func tryCache(c *gin.Context, cfg RateLimitConfig, attemptsKey, lockoutKey string) bool {
	ctx := c.Request.Context()

	// 1. Check hard lockout
	if cfg.LockoutThreshold > 0 {
		val, err := cacheStore.Get(ctx, lockoutKey)
		if err != nil && !errors.Is(err, core.ErrCacheKeyNotFound) {
			log.Printf("[rate-limit] Cache error checking lockout (%s): %v — falling back to in-memory", lockoutKey, err)
			return false
		}
		if val == "locked" {
	...
	countStr, err := cacheStore.Get(ctx, attemptsKey)
	if err != nil && !errors.Is(err, core.ErrCacheKeyNotFound) {
		log.Printf("[rate-limit] Cache error getting attempts (%s): %v — falling back to in-memory", attemptsKey, err)
		return false
	}
	...
	// 3. Increment (CacheStore.Increment sets TTL atomically on first create)
	newCount, err := cacheStore.Increment(ctx, attemptsKey, cfg.Window)
	if err != nil {
		log.Printf("[rate-limit] Cache error incrementing (%s): %v — falling back to in-memory", attemptsKey, err)
		return false
	}

	// 4. Check hard lockout threshold
	if cfg.LockoutThreshold > 0 && newCount >= cfg.LockoutThreshold {
		if err := cacheStore.Set(ctx, lockoutKey, "locked", cfg.LockoutDuration); err != nil {
			log.Printf("[rate-limit] Cache error setting lockout (%s): %v", lockoutKey, err)
		}
