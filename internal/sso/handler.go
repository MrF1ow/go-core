...
func NewHandler(adminRepo AdminRepository, userRepo *user.Repository, sessionService *session.Service) *Handler {
	return &Handler{
		AdminRepo:      adminRepo,
		UserRepo:       userRepo,
...
func (h *Handler) Exchange(c *gin.Context) {
	...
	if h.AppLookup != nil {
		if p, lookupErr := h.AppLookup(targetAppID); lookupErr == nil && p != nil {
			appPtr = p
		}
	}
