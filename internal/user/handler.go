...
func (h *Handler) Login(c *gin.Context) {
	...
	if h.BruteForceService != nil {
		if appPtr, lookupErr := h.Service.AppLookup(appID.String()); lookupErr == nil && appPtr != nil {
			bfCfg = bruteforce.ResolveConfig(appPtr)
		} else {
			// App not found or lookup error — use global defaults
...
func (h *Handler) RefreshToken(c *gin.Context) {
	...
	if claims, parseErr := jwt.ParseToken(req.RefreshToken); parseErr == nil && claims.AppID != "" {
		if appPtr, lookupErr := h.Service.AppLookup(claims.AppID); lookupErr == nil && appPtr != nil {
