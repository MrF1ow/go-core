...
func (s *Service) IsAccountLocked(user *models.User) (bool, *time.Time, error) {
	...
	if user.LockExpiresAt != nil && time.Now().UTC().After(*user.LockExpiresAt) {
		...
		if err := s.queries.ClearUserLockout(context.Background(), user.ID); err != nil {
...
func (s *Service) lockAccount(appID uuid.UUID, email string, cfg BruteForceConfig) (bool, *time.Time, error) {
	...
	rowsAffected, err := s.queries.LockUserByEmail(context.Background(), sqlcgen.LockUserByEmailParams{
	...
	if err != nil {
		return false, nil, fmt.Errorf("failed to lock user account: %w", err)
	}
	if rowsAffected == 0 {
		return false, nil, nil
	}
...
func (s *Service) UnlockAccount(appID uuid.UUID, userID uuid.UUID, email string) error {
	if err := s.queries.ClearUserLockout(context.Background(), userID); err != nil {
