...
// NewConfig builds a Config from the supplied values. redisNotifyEvents is the
// value of REDIS_NOTIFY_KEYSPACE_EVENTS; the constructor derives
// UseKeyspaceNotif from it.
func NewConfig(enabled bool, scanInterval time.Duration, keyspaceNotifEnabled bool, redisNotifyEvents string) Config {
	useKeyspaceNotif := redisNotifyEvents != "" && strings.Contains(redisNotifyEvents, "E")

	return Config{
		Enabled:              enabled,
		UseKeyspaceNotif:     useKeyspaceNotif,
		ScanInterval:         scanInterval,
		KeyspaceNotifEnabled: keyspaceNotifEnabled,
	}
}
...
func NewExpiryService(handler ExpiryHandlerInterface, config Config) *ExpiryService {
...
func (s *ExpiryService) listenForKeyExpirations() {
	...
	if client == nil {
		log.Println("[SessionGroup] PubSub unavailable: cache store is not Redis-backed")
		return
	}

	pubsub := client.PSubscribe(s.ctx, "__keyevent@0__:expired")
	defer pubsub.Close()
