...
// The pool parameter is used for writing log entries.
// The anomalyDetector may be nil (anomaly detection will be skipped).
func InitializeLogService(pool *pgxpool.Pool, anomalyDetector *AnomalyDetector) *Service {
...
func (s *Service) processLogEntry(entry LogEntry) {
	if s.pool == nil {
		log.Printf("Warning: Cannot process log entry, database pool is nil")
		return
	}
	...
	params := sqlcgen.CreateActivityLogParams{
		AppID:     entry.AppID,
		UserID:    entry.UserID,
	...
	for attempt := 1; attempt <= maxRetries; attempt++ {
		err := s.queries.CreateActivityLog(context.Background(), params)
		if err == nil {
			// Successfully logged
			return
		}
