...
func NewAnomalyDetector(pool *pgxpool.Pool, geoIPService *geoip.Service) *AnomalyDetector {
...
func (ad *AnomalyDetector) getUserPatterns(userID uuid.UUID, window time.Duration) (UserPatterns, error) {
	...
	// Query recent activity logs (only the fields we need)
	cutoffTime := time.Now().UTC().Add(-window)

	rows, err := ad.pool.Query(context.Background(),
		`SELECT ip_address, user_agent, timestamp FROM activity_logs
	...
	for rows.Next() {
		var r logRow
		if err := rows.Scan(&r.IPAddress, &r.UserAgent, &r.Timestamp); err != nil {
			return patterns, err
		}
		logs = append(logs, r)
	}
	...
	if ad.geoIP != nil && ad.geoIP.IsAvailable() {
		...
		ipRows, err := ad.pool.Query(context.Background(),
		...
		if err == nil {
			for ipRows.Next() {
				var ipAddr string
				if err := ipRows.Scan(&ipAddr); err == nil && ipAddr != "" {
					country := ad.geoIP.LookupCountry(ipAddr)
					if country != "" {
