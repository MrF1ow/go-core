...
func InitializeCleanupService(pool *pgxpool.Pool) *CleanupService {
...
func (cs *CleanupService) deleteExpiredLogs() (int64, error) {
	...
	for {
		...
		tag, err := cs.pool.Exec(context.Background(),
			`DELETE FROM activity_logs WHERE id IN (
				SELECT id FROM activity_logs WHERE expires_at IS NOT NULL AND expires_at < $1 LIMIT $2
			)`, now, batchSize)

		if err != nil {
			return totalDeleted, fmt.Errorf("failed to delete expired logs: %w", err)
		}

		deletedInBatch := tag.RowsAffected()
...
func (cs *CleanupService) DeleteLogsByUserID(userID string) (int64, error) {
	tag, err := cs.pool.Exec(context.Background(),
		`DELETE FROM activity_logs WHERE user_id = $1`, userID)

	if err != nil {
		return 0, fmt.Errorf("failed to delete logs for user %s: %w", userID, err)
	}

	log.Printf("Deleted %d activity logs for user %s", tag.RowsAffected(), userID)
...
func (cs *CleanupService) GetExpiredLogCount() (int64, error) {
	...
	err := cs.pool.QueryRow(context.Background(),
		`SELECT COUNT(*) FROM activity_logs WHERE expires_at IS NOT NULL AND expires_at < $1`, now,
	).Scan(&count)
...
func (cs *CleanupService) UpdateRetentionForEventType(eventType string, newRetentionDays int) (int64, error) {
	...
	tag, err := cs.pool.Exec(context.Background(),
		`UPDATE activity_logs
		 SET expires_at = timestamp + make_interval(days => $1)
		 WHERE event_type = $2 AND expires_at IS NOT NULL`,
		newRetentionDays, eventType)

	if err != nil {
		return 0, fmt.Errorf("failed to update retention for event type %s: %w", eventType, err)
	}

	log.Printf("Updated retention for %d logs of type %s", tag.RowsAffected(), eventType)
	return tag.RowsAffected(), nil
}
